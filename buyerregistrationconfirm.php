...
if (isset($_REQUEST['txtusername']) && isset($_REQUEST['txtpassword'])) {
	if (!empty($_REQUEST['txtusername']) && !empty($_REQUEST['txtpassword'])) {
		...
		if (mysql_num_rows($get_users) != 0) {
			echo "<script>alert('Username already exists, Please select other Username.');document.location='index.php?page=buyerregistration.php';</script>";
		}

		else {
			
			$query_login = "insert into tbllogin (username,password,securityquestion,securityanswer,usertype) values ('".$_REQUEST["txtusername"]."','".$_REQUEST["txtpassword"]."','".$_REQUEST["txtsecurityquestion"]."','".$_REQUEST["txtsecurityanswer"]."','buyer')";
